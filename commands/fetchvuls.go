...
var fetchVulsCmd = &cobra.Command{
	Use:   "vuls",
	Short: "Fetch CPE from Vuls",
	Long:  "Fetch CPE from Vuls",
	RunE:  fetchVuls,
}

func init() {
	fetchCmd.AddCommand(fetchVulsCmd)
}

func fetchVuls(_ *cobra.Command, _ []string) (err error) {
	...
	cpes, err := fetcher.FetchVuls()
	if err != nil {
		return xerrors.Errorf("Failed to fetch. err: %w", err)
	}
	if err := driver.InsertCpes(models.Vuls, cpes); err != nil {
